% ---------------------------------------------------------
% SIMULATION V33.0: 6-READER GRID & RADAR VISUALS
% Features: 
% 1. 6 Readers in a perfect grid (No Blind Spots)
% 2. "Radar Pulse" Animation (Looks high-tech, not messy)
% 3. All Logic Retained (Nurses, Beds, Equipment, Logs)
% ---------------------------------------------------------
clear; clc; close all;

disp('>> STARTING SIMULATION V33...');
disp('>> Status: 6 Readers Active. Radar System Online.');
...
% --- UPDATED: 6 READERS (GRID LAYOUT) ---
% This ensures 100% coverage of corners
rx = [3.5, 10, 16.5, 3.5, 10, 16.5]; 
ry = [11.5, 11.5, 11.5, 3.5, 3.5, 3.5]; 

% Animation Vars
pulseR = 0.5; maxPulse = 6.0; % Pulse radius
...
% 3. DASHBOARD UI
% =========================================================
f = figure('Name', 'RFID SYSTEM V33.0 (RADAR GRID)', 'Color', [0.05 0.05 0.05], ...
...
% --- LAYER 1: ANIMATED RADAR RINGS (Initialized) ---
hWaves = gobjects(1, length(rx));
for k = 1:length(rx)
    % Thin Cyan Circles
    hWaves(k) = rectangle('Position', [rx(k), ry(k), 0.1, 0.1], ...
        'Curvature', [1 1], 'EdgeColor', [0 1 1], 'LineWidth', 1, 'LineStyle', '-');
end
...
% 4. MAIN LOOP
% =========================================================
logs = {'\color{white}[SYS] 6-Reader Grid Online'; '\color{white}[SYS] Signal Scanning...'};
disp('>> Loop running...');
...
for t = 1:15000
    ...
    for k = 1:length(rx)
        % Updates the 6 circles
        set(hWaves(k), 'Position', [rx(k)-pulseR, ry(k)-pulseR, pulseR*2, pulseR*2]);
        % Dynamic Alpha (Fake it by toggling visibility or just keeping it thin)
        if pulseR > 5.0, set(hWaves(k), 'EdgeColor', [0 0.3 0.3]); else, set(hWaves(k), 'EdgeColor', [0 1 1]); end
    end
